...
namespace {
    ...
    static int Compare(const T& left, const T& right, bool is_min, const Comparator &com) {
...
class ReferenceVarRef : public VarRef<TValue> {
private:
    ...
    template <class T>
    static std::vector<VarRef<T>*> dedup_sources(const std::vector<VarRef<T>*>& in) {
    ...
    void OnValueChange(const TValue &value) {
        this->set_value(value);
    }
    struct IBinderBase {
        virtual ~IBinderBase() = default;
        virtual bool OwnsSource([[maybe_unused]]const void* p) const noexcept { return false; }
    };
    struct BinderToken {
        void* self = nullptr;
        ...
        BinderToken& operator=(BinderToken&& o) noexcept {
            if (this != &o) { reset(); self=o.self; unsubscribe=o.unsubscribe; id=o.id;
                              o.self=nullptr; o.unsubscribe=nullptr; o.id=0; }
            return *this;
        }
        ~BinderToken(){ reset(); }
        void reset() noexcept { if (self && unsubscribe && id) { unsubscribe(self,id); self=nullptr; id=0; } }
        BinderToken(const BinderToken&) = delete;
        BinderToken& operator=(const BinderToken&) = delete;
    };
    template<typename Derived>
    class IBinder : public IBinderBase{
    private:
    ...
    public:
        ...
        const std::vector<std::pair<std::size_t, CallbackType>>& ValueChanged() const noexcept {
        ...
        void UnBindSubscribe(std::size_t id) {
            if (value_changed_callbacks_.empty() || destroyed_) return;
            auto it = std::find_if(value_changed_callbacks_.begin(), value_changed_callbacks_.end(),
    ...
    template<typename T>
    class Binder : public IBinder<Binder<T>> {
    private:
        ...
        void OnValueChanged(const VarRef<T>&, const T&, const T& new_value) {
            ...
            for (auto &[_, callback] : callbacks) {
                if (BaseType::Destroyed()) break;
                callback(new_value);
            }
        }
    public:
        static_assert(std::is_same_v<T, TValue>, "Single-source Binder<T> must match ReferenceVarRef<TValue>");
        bool OwnsSource(const void* p) const noexcept override { return p == source_; }
        explicit Binder(VarRef<T>* source) : source_(source) {
            auto cb = source_->subscribe_value_changed([this](const VarRef<T>& var_ref, const T &old_value, const T &new_value){
                OnValueChanged(var_ref, old_value, new_value);
            });
            binder_subscribes_.emplace_back(std::move(cb));
        }
        ~Binder() {
            binder_subscribes_.clear();
        }
    };
    ...
    class FunctionBinder : public IBinder<FunctionBinder<F, Ts...>> {
                ...
                [this](const VarRef<Ti>&, const Ti&, const Ti& nv) {
                    ...
                    TValue out = std::apply(
                        [this](auto&... xs) -> TValue {
                            ...
                            static_assert(std::is_convertible_v<R, TValue>, "func result must be convertible to TValue");
                            return static_cast<TValue>(std::invoke(func_, xs...));
                        }, cache_);
                    auto callbacks = BaseB::ValueChanged();
                    for (auto& [_, cb] : callbacks) {
                        if (BaseB::Destroyed()) break;
                        cb(out);
                    }
                }
            );
        ...
        void AttachAll(std::index_sequence<Is...>) { (AttachOne<Is>(), ...); }
    ...
    private:
        ...
        void OnValueChanged(const VarRef<TSource>&, const TSource& old_value, const TSource& new_value) {
            auto callbacks = BaseType::ValueChanged();
            for (auto &[_, callback] : callbacks) {
                if (BaseType::Destroyed()) break;
    ...
    public:
        static_assert(std::is_same_v<TSource, TValue>, "AggregateBinder<T, TSource> must match ReferenceVarRef<TValue>");
        bool OwnsSource(const void* p) const noexcept override {
            for (auto* s : sources_) if (s == p) return true;
            return false;
        }
        explicit AggregateBinder(T seed, 
            std::function<T(const TSource&)> selector, 
            const std::vector<VarRef<TSource> *>& source) 
                : value_(seed), 
                  selector_(std::move(selector)),
                  sources_(source){
                for (auto* it : sources_) {
                    auto cb = it->subscribe_value_changed([this](const VarRef<TSource>& var_ref, const TSource& old_value, const TSource& new_value) {
                        OnValueChanged(var_ref, old_value, new_value);
                    });
    ...
    void OnValueChanged(const VarRef<T>&, const T& old_value, const T& new_value) {
           if (Compare(old_value, new_value, is_min_, comparison_) > 0) {
               if (Compare(value_, new_value, is_min_, comparison_) <= 0) return;
               value_ = new_value;
               auto callbacks = BaseType::ValueChanged();
               for (auto& [_, cb] : callbacks) { 
                    if (BaseType::Destroyed()) break; 
           ...
           if (Compare(value_, old_value, is_min_, comparison_) >= 0) {
               ...
               for (std::size_t i=1;i<sources_.size();++i) {
                   ...
                   if (Compare(v, best, is_min_, comparison_) < 0) best = v;
               }
               if (Compare(best, value_, is_min_, comparison_) != 0) {
                   value_ = best;
                   auto callbacks = BaseType::ValueChanged();
                   for (auto& [_, cb] : callbacks) { if (BaseType::Destroyed()) break; cb(value_); }
               }
           }
    }
    public:
        static_assert(std::is_same_v<T, TValue>, "MinMaxBinderSlim<T, Comparator> must match ReferenceVarRef<TValue>");
        bool OwnsSource(const void* p) const noexcept override {
            for (auto* s : sources_) if (s == p) return true;
            return false;
        }
        ...
        explicit MinMaxBinderSlim(bool is_min, T value, 
            ...
            const std::vector<VarRef<T>*>& source) 
                ...
                if (sources_.empty()) {
                    ...
                    for (std::size_t i=1;i<sources_.size();++i) {
                         const auto v = sources_[i]->value();
                        if (Compare(v, value_, is_min_, comparison_) < 0) value_ = v;
                    }
                }
                for (auto* it : sources_) {
                    auto cb = it->subscribe_value_changed([this](const VarRef<T>& var_ref, const T& old_value, const T& new_value) {
                        OnValueChanged(var_ref, old_value, new_value);
                    });
    ...
    private:
        ...
        void OnValueChanged(const VarRef<T>& varRef, const T& old_Value, const T& new_Value) {
            auto comp = Compare(old_Value, new_Value, is_min_, comparison_);
            if (comp == 0) return;
    
            auto it = indexs_.find(const_cast<VarRef<T>*>(&varRef));
            if (it == indexs_.end()) return;
            std::size_t n = it->second;
    
            if (comp > 0) {
                // 变“更优”（min: 变小 / max: 变大）→ 上滤
                auto* target = heap_[n];
                for (std::size_t n2 = (n - 1) >> 1; n > 0
                     && Compare(target->value(), heap_[n2]->value(), is_min_, comparison_) < 0;
                     n2 = ((n = n2) - 1) >> 1) {
                    indexs_[heap_[n] = heap_[n2]] = n;
                }
                indexs_[heap_[n] = target] = n;
                if (n != 0) return; // 未到顶无需通知
                // 到顶，值肯定发生了“更优”变化，直接通知
            } else {
                // 变“更差”（min: 变大 / max: 变小）→ 下滤
                const bool nottop = (n != 0);
                auto* target = heap_[n];
                for (std::size_t n2 = (n << 1) + 1, len = heap_.size(); n2 < len; n2 = ((n = n2) << 1) + 1) {
                    if (n2 + 1 < len && Compare(heap_[n2 + 1]->value(), heap_[n2]->value(), is_min_, comparison_) < 0) ++n2;
                    if (Compare(heap_[n2]->value(), target->value(), is_min_, comparison_) >= 0) break;
                    indexs_[heap_[n] = heap_[n2]] = n;
                }
                indexs_[heap_[n] = target] = n;
                if (nottop) return; // 非顶点无须通知
    
                // 顶点可能换人，但顶点“值”不一定变；若没变则不通知
                const T& new_top = heap_[0]->value();
                if (tri_cmp(comparison_, new_top, old_Value) == 0) return;
            }
    ...
    public:
    static_assert(std::is_same_v<T, TValue>, "MinMaxBinder<T, Comparator> must match ReferenceVarRef<TValue>");
    bool OwnsSource(const void* p) const noexcept override {
        return indexs_.find(reinterpret_cast<VarRef<T>*>(const_cast<void*>(p))) != indexs_.end();
    }
        ...
        explicit MinMaxBinder(bool is_min, Comparator comparison, 
            ...
            for (std::size_t i = 0; i < heap_.size(); i++) {
                auto cb = heap_[i]->subscribe_value_changed([this](const VarRef<T>& var_ref, const T& old_value, const T& new_value){
                    OnValueChanged(var_ref, old_value, new_value);
                });
                subscribes_.emplace_back(std::move(cb));
            }
            for (std::size_t i = 0; i < heap_.size(); ++i) indexs_[heap_[i]] = i;
...
public:
    ReferenceVarRef(TValue default_value, ComparerType com = {}) : BaseType(default_value, std::move(com)) {}
    ~ReferenceVarRef() {
       UnBind();
    }
    bool Bind(std::function<std::tuple<std::unique_ptr<IBinderBase>, BinderToken, TValue>()> factory) {
        if (binder_) return false;
    ...
    bool UnBind(VarRef<TValue> *var_ref, TValue value) {
        if (!UnBind(var_ref)) return false;
        this->set_value(value);
        return true;
    }
    ...
    return Bind([&]{
      ...
      auto tok = b->BindSubscribe([this](const TValue& v){ OnValueChange(v); });
 ...
 template<class F, class... Ts>
    bool BindFunc(F&& func, VarRef<Ts>&... srcs) {
...
bool BindAggregate(TValue seed, Selector selector, const std::vector<VarRef<TSource>*>& sources) {
    auto uniq = dedup_sources(sources);
    return Bind([&]{
        using B = AggregateBinder<TValue, TSource>;
        TValue init = seed;
        for (auto* s : uniq) init = init + selector(s->value());
        auto b   = std::make_unique<B>(init, std::function<TValue(const TSource&)>(selector), uniq);
        auto tok = b->BindSubscribe([this](const TValue& v){ OnValueChange(v); });
        return std::make_tuple(std::unique_ptr<IBinderBase>(std::move(b)), std::move(tok), init);
    });
}

template <class Cmp = std::less<TValue>>
bool BindMin(Cmp cmp, const std::vector<VarRef<TValue>*>& sources) {
    return BindMinMaxAuto(/*is_min*/true, std::move(cmp), sources, kMinMaxHeapSlimThreshold);
}

template <class Cmp = std::less<TValue>>
bool BindMax(Cmp cmp, const std::vector<VarRef<TValue>*>& sources) {
    return BindMinMaxAuto(/*is_min*/false, std::move(cmp), sources, kMinMaxHeapSlimThreshold);
}

template <class Cmp = std::less<TValue>>
    bool BindMinMaxAuto(bool is_min, Cmp cmp, const std::vector<VarRef<TValue>*>& sources,
        ...
        if (uniq.size() > slim_threshold) {
            return Bind([&]{
                ...
                for (size_t i=1;i<uniq.size();++i) {
                    if (is_min ? cmp(uniq[i]->value(), init) : cmp(init, uniq[i]->value()))
                        init = uniq[i]->value();
                }
                auto b   = std::make_unique<B>(is_min, std::move(cmp), uniq);
                auto tok = b->BindSubscribe([this](const TValue& v){ this->set_value(v); });
                return std::make_tuple(std::unique_ptr<IBinderBase>(std::move(b)), std::move(tok), init);
            });
        } else {
            return Bind([&]{
                ...
                for (size_t i=1;i<uniq.size();++i)
                    if (is_min ? (tri_cmp(cmp, uniq[i]->value(), init) < 0)
                        ...
                        init = uniq[i]->value();
                auto b   = std::make_unique<B>(is_min, this->value(), std::move(cmp), uniq);
                auto tok = b->BindSubscribe([this](const TValue& v){ OnValueChange(v); });
                return std::make_tuple(std::unique_ptr<IBinderBase>(std::move(b)), std::move(tok), init);
            });
        }
