...
public:
    class fulfillsubscribe {
        private:
            ...
            explicit fulfillsubscribe(RequirementMonitor* ptr, std::size_t id, std::weak_ptr<bool> alive) noexcept
    ...
    void unsubscribe_fulfilled_state_changed(std::size_t id) {
        ...
        auto it = std::find_if(listeners_.begin(), listeners_.end(), [id](const auto& p) {return p.first == id;});
...
namespace detail {
        ...
        protected:
            ...
            BinaryRequirementMonitor(ValueType left, ValueType right) 
                : left_(std::move(left)), right_(std::move(right)) {
                    if (!left_ || !right_) {
                        throw std::invalid_argument("BinaryRequirementMonitor: left and right monitors cannot be null");
                    }
            }
            ...
            void on_start() override {
                left_->start();
                right_->start();
                left_subscribe_ = left_->subscribe_fulfilled_state_changed([this](RequirementMonitor&) {
                    this->OnStateChange();
                });
                right_subscribe_ = right_->subscribe_fulfilled_state_changed([this](RequirementMonitor&) {
                    this->OnStateChange();
                });
                set_fulfilled(UpdateFulfilledState());
            }
    ...
    class NotRequirementMonitor : public RequirementMonitor {
        ...
        protected:
            void on_start() override {
                base_monitor_->start();
                base_subscribe_ = base_monitor_->subscribe_fulfilled_state_changed([this](RequirementMonitor&){
                    this->OnStateChange();
                });
                set_fulfilled(!base_monitor_->Fulfilled());
            }
        ...
        public:
            NotRequirementMonitor(ValueType base_monitor) 
                : base_monitor_(std::move(base_monitor)) {
                    if (!base_monitor_) {
                        throw std::invalid_argument("NotRequirementMonitor: base monitor cannot be null");
                    }
                }
...
std::unique_ptr<RequirementMonitor> make_and(Monitors&& ...monitor) {
    static_assert(sizeof...(monitor) >= 2, "AND operation requires at least 2 monitors");
    return (std::forward<Monitors>(monitor) & ...);
}

template<typename ...Monitors>
std::unique_ptr<RequirementMonitor> make_or(Monitors&& ...monitor) {
    static_assert(sizeof...(monitor) >= 2, "OR operation requires at least 2 monitors");
    return (std::forward<Monitors>(monitor) | ...);
}
...
inline std::unique_ptr<RequirementMonitor> operator&(std::unique_ptr<RequirementMonitor>&& left, 
                                             std::unique_ptr<RequirementMonitor>&& right) {
    if (!left) return right;
    if (!right) return left;
    return std::make_unique<detail::AndRequirementMonitor>(std::move(left), std::move(right));
}
...
inline std::unique_ptr<RequirementMonitor> operator|(std::unique_ptr<RequirementMonitor>&& left, 
    ...
    if (!left) return right;
    if (!right) return left;
    return std::make_unique<detail::OrRequirementMonitor>(std::move(left), std::move(right));
}
...
inline std::unique_ptr<RequirementMonitor> operator^(std::unique_ptr<RequirementMonitor>&& left, 
    ...
    if (!left) return right;
    if (!right) return left;
    return std::make_unique<detail::XorRequirementMonitor>(std::move(left), std::move(right));
}

inline std::unique_ptr<RequirementMonitor> operator!(std::unique_ptr<RequirementMonitor> && m) {
