...
template<typename TIndex>
class Transition : public Transitable<TIndex> {
protected:
    using HandlerCallbackType = std::function<void (const TIndex&, const TIndex&)>;
...
public:
    explicit Transition(State<TIndex> &departure, State<TIndex> &destination,
         RequirementMonitor* requirement, HandlerCallbackType callback, bool enter_history)
        : departure_(&departure), destination_(&destination), 
          requirement_(requirement),  handler_(std::move(callback)), enter_history_(enter_history) {}
    virtual ~Transition() = default;

    bool enter_history_;
    void enter() {
        if (entered_.exchange(true)) return;
        if (requirement_) {
            requirement_->start();
            auto subscribe = requirement_->subscribe_fulfilled_state_changed([this](RequirementMonitor& requirement){
                OnRequirementStateChanged(requirement);
            });
            holdfulfillsubscribe_ = std::make_unique<RequirementMonitor::fulfillsubscribe>(std::move(subscribe));
    ...
    void exit() {
        if (!entered_.exchange(false)) return;
