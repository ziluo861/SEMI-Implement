...
private:
    ...
    std::unique_ptr<StateMachineEnterExitTransitable<TIndex>> fsm_enter_exit_transtions_;
    ...
    static State<TIndex>* FindLCA(State<TIndex>* departure, State<TIndex>* destination) {
    ...
    while (!departureStack.empty() && !destStack.empty()) {
        ...
        if (_departstate != _deststate) break;

        lca = _deststate;
    }
    if (!lca) throw std::runtime_error("Departure and destination not in the same tree");
    return lca;
}
...
public:
    class fsmsubscripte {
        ...
        private:
            explicit fsmsubscripte(FSM *ptr, std::size_t id) noexcept
                : ptr_(ptr), id_(id), version_(ptr ? ptr->version_ : 0) {}
            friend class FSM<TIndex>;
            void release() noexcept {
                if (ptr_ && !ptr_->destroyed_ && ptr_->version_ == version_) {
    ...
    fsmsubscripte subscribe(CallbackType callback) {
        ...
        return fsmsubscripte(this, _size);
    }
    void unsubscribe(std::size_t id) {
        if (destroyed_ || transition_occured_.empty()) return;
        auto it = std::find_if(transition_occured_.begin(), transition_occured_.end(), [id](const auto &p) {return p.first == id;});
        if (it != transition_occured_.end()) {
            transition_occured_.erase(it);
        }
    }
    explicit FSM(TIndex entrance_index, std::vector<std::unique_ptr<State<TIndex>>> states) : FSM(entrance_index, state_handler{}, std::move(states)){}
    explicit FSM(TIndex entrance_index, state_handler handler, std::vector<std::unique_ptr<State<TIndex>>> states) : version_(1) {
    root_state_ = std::make_unique<State<TIndex>>(entrance_index, std::move(handler), std::move(states));
    ...
    while (!sta.empty()) {
        ...
        if (cur_state->StateMachine()) {
            throw std::runtime_error("The Child Being Added Belongs To Another Finite State Machine");
        } 
    ...
    fsm_enter_exit_transtions_ = std::make_unique<StateMachineEnterExitTransitable<TIndex>>(this);
    auto _transtions_subscriptes =  fsm_enter_exit_transtions_->subscribe_block_state_change([this](Transitable<TIndex>& transitable){
        OnBlockStateChange(&transitable);
    });
    hold_exit_sub_ = std::make_unique<ExitSub>(std::move(_transtions_subscriptes));       
    }
    ~FSM() {
        destroyed_ = true;
        ++version_;
    ...
    if (state->Parent() && 
        (state == state->Parent()->EntranceState() 
        || state == state->Parent()->ExitusState())) {
            state = state->Parent();
    }
    if (current_state_ == state) return;
    auto old_index = current_state_ ? current_state_->Index() : root_state_->Index();
    current_state_ = state;
    if (old_index == state->Index()) return;
    ...
    void AppendTransition(TIndex departure, TIndex destination, RequirementMonitor *requirement, CallbackType transitionHandler, bool enterHistory) {
    if (running_) {
        throw  std::runtime_error("Finite State Machine Cannot Append Transition On Running!");
    }
    std::unique_ptr<Transition<TIndex>> transition;
    std::unique_ptr<typename Transitable<TIndex>::blocksubscribe> holdsubscripte;
    if (departure == destination) {
        auto it = all_states_.find(departure);
        if (it == all_states_.end()) throw std::runtime_error("Not found departure");
        State<TIndex>* state = it->second;
        transition = std::make_unique<SelfTransition<TIndex>>(*state, requirement, std::move(transitionHandler), enterHistory);
        auto subscription = transition->subscribe_block_state_change([this](Transitable<TIndex>& transitable){
            OnBlockStateChange(&transitable);
        });
        holdsubscripte = std::make_unique<typename Transitable<TIndex>::blocksubscribe>(std::move(subscription));
    ...
    if (it == all_states_.end()) throw std::runtime_error("Not found destination");
    ...
    transition = std::make_unique<CrossBranchingTransition<TIndex>>(*depart_state, *dest_state, *lca, requirement, std::move(transitionHandler), enterHistory);
    auto subscription = transition->subscribe_block_state_change([this](Transitable<TIndex>& transitable){
            OnBlockStateChange(&transitable);
    });
    holdsubscripte = std::make_unique<typename Transitable<TIndex>::blocksubscribe>(std::move(subscription));
    depart_state->addtransition(std::move(transition), std::move(holdsubscripte));
    }
