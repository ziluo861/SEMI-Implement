...
public:
  ...
  Self *HistoryState() noexcept {
    return history_state_ ? history_state_ : entrance_state_;
  }
  const Self *HistoryState() const noexcept {
    return history_state_ ? history_state_ : entrance_state_;
  }
  ...
  void UpdateStateMachine(FSM<TIndex> *fsm) {
    statemachine_ = fsm;
    if (exitus_state_)
      exitus_state_->statemachine_ = fsm;
    if (entrance_state_)
      entrance_state_->statemachine_ = fsm;
  }
  ...
  void Exit() {
    if (!running_.value() || exiting_.exchange(true))
      return;
      ...
      for (auto &state : child_states) {
        ...
        if (state->parent_) {
          throw std::runtime_error(
              "The Child Being Added Belongs To Another State");
        }
        auto key = state->index_;
        state->parent_ = this;
        auto [it, ok] = child_states_.emplace(key, std::move(state));
        if (!ok)
          throw std::runtime_error("Duplicate child index");
      }
