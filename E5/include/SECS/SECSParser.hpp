...
private:
    ...
    static bool GetItemWithoutContent(std::string_view raw, std::string &context, std::string& base) noexcept {
...
public:
    static ParseResult TryParseContent(std::string_view _Raw) {
        ...
        if (_size < 2 || _Trim[0] != RangeStartMark || _Trim[_size - 1] != RangeEndMark) {
            return std::nullopt;
        }
    ...
    static bool TryDeserialize(std::span<std::uint8_t>& bytes, ParseResult& _item) {
        _item.reset();
        if (bytes.empty()) {
            return true;
        }
        auto bytesFormat = bytes[0];
        int lengthBytesLength = static_cast<int>(bytesFormat & LengthBytesCountFilter);
        if (lengthBytesLength == 0 || lengthBytesLength >= static_cast<int>(bytes.size())) {
            return false;
        }
        _item = SECSFactory::createItem(static_cast<FormatCode>(bytesFormat & FormatCodeFilter));
        if (!_item.has_value()) {
            return false; 
        }
