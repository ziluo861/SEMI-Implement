...
public:
    FormatCode GetFormat() const noexcept {
        return FormatCode::ListFormatCode;
    }
    ...
    bool ParseContent(std::string_view _context) {
        ...
        for (int i = 0; i < _len; i++) {
            if (_context[i] == RangeStartMark) {
                ...
                count -= 1;
                if (count == 0) {
                    auto _item = SECSParser::TryParseContent(_context.substr(lastIndex + 1, i - lastIndex));
                    if (!_item.has_value()) {
                        return false;
                    }
            ...
            } else if (count == 0 && !std::isspace(static_cast<unsigned char>(_context[i]))) {
                return false;
            }
        }
        return count == 0;
