...
public:
    ...
    [[nodiscard]]
    static inline std::vector<std::string_view> SplitAndRemoveEmpty(std::string_view str) noexcept {
        if (str.empty()) {
            return {};
        }
    ...
    static inline bool IsNullOrWhiteSpace(std::string_view str) noexcept {
        // return str.empty() || std::all_of(str.begin(), str.end(), [](unsigned char _char) {
  ...
  template<typename T>
    ...
    static std::string DeparseValue(T value) {
        if constexpr(std::is_same_v<T, bool>) {
            ...
            static_assert(std::is_arithmetic_v<T>, "Unsupported type for DeparseValue");
        }
    } 
private:
    template<typename T>
    ...
    static std::string DeparseIntegral(T value) {
        static_assert(std::is_integral_v<T> && !std::is_same_v<T, bool>, "Only supports integral types");
    ...
    static std::string DeparseFloating(T value, int Precision = 6) {
        ...
        std::to_chars_result result = std::to_chars(buffer.data(), buffer.data() + buffer.size(), 
