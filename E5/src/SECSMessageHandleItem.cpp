...
SECSItemBase* SECSMessageHandleItem::RejectItem() noexcept {
 ...
 const std::vector<std::uint8_t>& SECSMessageHandleItem::RemoteHeadBytes() const noexcept{
    return remote_head_bytes_;
 }

 SECSHead SECSMessageHandleItem::ReplyHead() noexcept {
        return SECSHead(request_head_->Stream(), 
                    static_cast<std::uint8_t>(reply_item_ == RejectItem() ? 0u : request_head_->Function() + 1u), 
                    false);
 }
