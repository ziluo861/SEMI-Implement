...
SECSHead::SECSHead(valuetype streamByte, valuetype functionByte) {
    stream_byte_ = static_cast<valuetype>(((functionByte & 1u) == 0u) ? (streamByte & stream_filter_) : streamByte);
    function_byte_ = functionByte;
}

bool SECSHead::CheckRequestReply(valuetype reqStreamByte, valuetype reqFunctionByte, 
                               valuetype resStreamByte, valuetype resFunctionByte) noexcept {
